...
export default async function RunwayShowsPage() {
  const shows = await getRunwayShows();
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
            Manage your runway show collections
          </p>
        </div>
        <Link
          href="/admin/runway-shows/new"
      ...
      {shows.length === 0 ? (
        <div className="rounded-lg border border-dashed border-gray-300 bg-white p-12 text-center dark:border-gray-700 dark:bg-gray-900">
          <p className="text-gray-500 dark:text-gray-400">
            No runway shows yet. Create your first one!
          </p>
          <Link
            href="/admin/runway-shows/new"
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
              {shows.map((show) => (
                ...
                >
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                    {show.themes?.length || 0} themes
                  </td>
                  <td className="whitespace-nowrap px-6 py-4 text-right text-sm">
                    <div className="flex items-center justify-end gap-2">
                      <Link
                        href={`/admin/runway-shows/${String(show.id)}`}
